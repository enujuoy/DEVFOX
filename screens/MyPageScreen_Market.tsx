// screens/MyPageScreen_Market.tsx
...
type NavProp = StackNavigationProp<MyPageMarketStackParamList, 'Market'>;

interface EventItem { id: string; title: string; period: string; }

export default function MyPageScreen_Market() {
  const navigation = useNavigation<NavProp>();
  const [events, setEvents] = useState<EventItem[]>([
    { id: '1', title: '전상품 30% 할인 이벤트', period: '2025.04.01～2025.05.01' },
  ]);
  const [modalVisible, setModalVisible] = useState(false);
  const [toDeleteId, setToDeleteId] = useState<string | null>(null);

  const onPressAdd = () => navigation.navigate('EventManagement');
        ...
        <Modal transparent visible={modalVisible} animationType="fade">
          <View style={{
            ...
            <View style={{
              ...
              <Text style={{fontSize:16,textAlign:'center'}}>정말 삭제하시겠습니까?</Text>
              <View style={{
                ...
                <TouchableOpacity onPress={confirmDelete}
                  style={{padding:8,backgroundColor:'#d00',borderRadius:6}}>
                  <Text style={{color:'#fff'}}>예</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
